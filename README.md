...
| **(new)** no-401-if-no-www-authenticate | Never implement a response with the status code `401 Unauthorized` if it does not return a `WWW-Authenticate` header. | RFC 9110, Section 15.5.2 (401 Unauthorized), RFC 9110, Section 11.6.1 (WWW-Authenticate) |
